...
function isWithinActiveRange(from,to){ const now = nowHM(); if(from<=to) return now>=from && now<=to; return now>=from || now<=to; }
...
// ---------------- DISPATCHER ----------------
let job = null;
function startDispatcher(){ stopDispatcher(); job = schedule.scheduleJob('*/1 * * * *', async ()=>{
...
function importPairingCode(code){
  ...
  delete store.pairingCodes[code]; saveStore();
  return newId;
}
...
// ---------------- START ----------------
if(store.settings.enabled) startDispatcher();function makeId(){ return shortid.generate(); }
function nowHM(){ const d=new Date(); return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; }
